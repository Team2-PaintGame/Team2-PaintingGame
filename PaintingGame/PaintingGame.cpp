...
void PaintingGame::InitialiseAssets() {
	...
	meshes.insert(std::make_pair("quadMesh", renderer->LoadQuad()));
	meshes.insert(std::make_pair("emitterMesh", renderer->LoadMesh("emitter.msh")));
...
void PaintingGame::InitWorld() {
	...
	world->AddGameObject(new ParticleSystem<InkParticle>(physicsCommon, physicsWorld, Vector3(0, 10, 0), 45.0f, meshes.at("sphereMesh"), textures.at("grassTex"), shaders.at("instancedShader"), false, 1));
}
