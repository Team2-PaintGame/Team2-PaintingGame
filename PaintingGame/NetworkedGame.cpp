...
void NetworkedGame::ServerCreateClientPlayer(SpawnPacket* payload)
{
	...
	ClientPlayer = CreatePlayer(payload->position, PlayerBase::Team::Blue);
...
void NetworkedGame::ClientCreateServerPlayer(SpawnPacket* payload)
{
	...
	ServerPlayer = CreatePlayer(payload->position, PlayerBase::Team::Red);
...
NCL::Player* NetworkedGame::SpawnPlayer() {
	if (thisServer) {
		ServerPlayer = AddPlayer(Vector3(5.0f, 15.0f, 5.0f), PlayerBase::Team::Red);
		ServerPlayerID = 1;
		return ServerPlayer;
	}
	if (thisClient) {
		// send to server that player has been spawned
		ClientPlayer = AddPlayer(Vector3(-5.0f, 10.0f, -5.0f), PlayerBase::Team::Blue);
		ClientPlayerID = 2;
		SpawnPacket packet;
		packet.position = ClientPlayer->GetTransform().GetPosition();
		packet.playerID = ClientPlayerID;
...
Player* NetworkedGame::AddPlayer(Vector3 position, PlayerBase::Team team) {
